...
import shutil
#%%
folder_path='D:/data/bone_marrow/BM_cytomorphology_data'
out_path='D:/data/bone_marrow/BM_class'
for folder in os.listdir(folder_path):
    try:
        os.mkdir(os.path.join(out_path,folder))
    except:
        pass
    if len(glob.glob(os.path.join(folder_path,folder,'*.jpg')))==0:
        for sub_folder  in os.listdir(os.path.join(folder_path,folder)):
            ...
            for file in os.listdir(os.path.join(folder_path,folder,sub_folder)):
                shutil.copy(os.path.join(folder_path,folder,sub_folder,file), os.path.join(out_path,folder,file))
    else:
        for file in os.listdir(os.path.join(folder_path,folder)):
            shutil.copy(os.path.join(folder_path,folder,file), os.path.join(out_path,folder,file))
#%%
for fn in os.listdir(out_path):
    print(fn+':'+str(len(os.listdir(os.path.join(out_path,fn)))))


#%% 
aug_num=30000
train_rate=0.8
dataset="D:/data/bone_marrow/training/BM_class"
trainset="D:/data/bone_marrow/training/BM_class_20220127"
try:
...
for fn in os.listdir(dataset):
    ...
    for imn in os.listdir(os.path.join(dataset,fn)):
        ...
        if train_rate<np.random.rand(1):
            ...
            cv2.imwrite(os.path.join(trainset,'test',fn,fnn+"_{:04d}".format(0)+'.jpg'),im)
            # data_num-=1
            continue
        ...
        fcount=1
        im=cv2.imread(os.path.join(dataset,fn,imn))
        cv2.imwrite(os.path.join(trainset,'train',fn,fnn+"_{:04d}".format(0)+'.jpg'),im)
        im=cv2.imread(os.path.join(dataset,fn,imn))
        for i in range(aug_rate):
            if aug_num/data_num>np.random.rand(1):
                ...
                cv2.imwrite(os.path.join(trainset,'train',fn,fnn+"_{:04d}".format(fcount)+'.jpg'),ima)
