...
class Attn_Net_Gated(nn.Module):
    def __init__(self, L = 500, D = 256, dropout = False, n_classes = 5):
...
class ResNet(nn.Module):
    def __init__(
        ...
        attention_net = Attn_Net_Gated(L = size[1], D = size[2], dropout = False, n_classes = 5)

        fc.append(attention_net)
        self.attention_net = nn.Sequential(*fc)
        self.n_classes=num_classes
        
        bag_classifiers = [nn.Linear(size[1], 1) for i in range(self.n_classes)] #
...
def resnext101_32x4d(progress: bool = True, backbone_path: str='', backnone_fc: int=21, num_classes: int=5, **kwargs: Any) -> ResNet:
    ...
    Args:
        backnone_fc (str): Path to pretrained CNN backbone
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    kwargs["groups"] = 32
    kwargs["width_per_group"] = 4
    return _resnet("resnext101_32x4d", Bottleneck, [3, 4, 23, 3],  backbone_path, backnone_fc, num_classes, progress, **kwargs)
